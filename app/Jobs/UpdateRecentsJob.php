...
class UpdateRecentsJob implements ShouldQueue
{
    ...
    public function handle()
    {
        if($this->type === 'movie') {
            ...
            $recent->touch();
            $keep = Recent_series::where('user_id', $this->userId)->latest('updated_at')->take(config('constants.recently_viewed.latest_n'))->pluck('id');
            ...
            $recent->touch();
            $keep = Recent_person::where('user_id', $this->userId)->latest('updated_at')->take(config('constants.recently_viewed.latest_n'))->pluck('id');
            ...
            $recent->touch();
            $keep = Recent_user::where('user_id', $this->userId)->latest('updated_at')->take(config('constants.recently_viewed.latest_n'))->pluck('id');
            ...
            $recent->touch();
            $keep = Recent_list::where('user_id', $this->userId)->latest('updated_at')->take(config('constants.recently_viewed.latest_n'))->pluck('id');
