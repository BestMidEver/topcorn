...
class SuckDataJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach(Series_rated::All()->pluck('movie_id')->unique() as $id){
            SuckSeriesJob::dispatch($id, true)->onQueue("low");
        }
        foreach(Series_later::All()->pluck('movie_id')->unique() as $id){
            SuckSeriesJob::dispatch($id, false)->onQueue("low");
        }
        foreach(Series_ban::All()->pluck('movie_id')->unique() as $id){
            SuckSeriesJob::dispatch($id, false)->onQueue("low");
        }
