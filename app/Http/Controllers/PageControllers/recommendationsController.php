...
class recommendationsController extends Controller
{
    ...
    public function get_pemosu(Request $request)
    {
        ...
        ->havingRaw('sum((rateds.rate-3)*recommendations.is_similar) DIV '.count($request->f_users).' > 7 AND sum(rateds.rate-1)*25 DIV COUNT(recommendations.this_id) > 74 AND sum(IF(r2.id IS NULL OR r2.rate = 0, 0, 1)) = 0');
