...
class recommendationsController extends Controller
{
    ...
    public function get_pemosu(Request $request)
    {
        ...
        ->select(
            ...
            'movies.'.$hover_title.' as original_title',
            DB::raw('sum(IF(recommendations.is_similar, 0, 7)*(rateds.rate-3.8))*movies.vote_average AS point'),
        ...
        ->groupBy('movies.id')
        ->havingRaw('sum(IF(recommendations.is_similar, 0, 7)*(rateds.rate-3.8)) > 4 AND sum(IF(r2.id IS NULL OR r2.rate = 0, 0, 1)) = 0')
        ->orderBy('point', 'desc');
