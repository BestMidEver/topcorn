...
class recommendationsController extends Controller
{
    ...
    public function get_pemosu(Request $request)
    {
        ...
        ->select(
            ...
            'movies.'.$hover_title.' as original_title',
            DB::raw('sum((rateds.rate-3)*(1/(recommendations.is_similar+4))) AS point'),
        ...
        ->groupBy('movies.id')
        ->havingRaw('sum(rateds.rate-3) > 1 AND sum(rateds.rate-3)/COUNT(movies.id) > 1.5 AND sum(IF(r2.id IS NULL OR r2.rate = 0, 0, 1)) = 0')
        ->orderBy('point', 'desc');
