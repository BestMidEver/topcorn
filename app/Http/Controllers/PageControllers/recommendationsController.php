...
class recommendationsController extends Controller
{
    ...
    public function get_pemosu(Request $request)
    {
        ...
        ->select(
            ...
            DB::raw('100 - (100 - (sum(rateds.rate*recommendations.is_similar)*4 / COUNT(movies.id)) DIV 2) as percent'),
        ...
        ->groupBy('movies.id')
        ->havingRaw('sum((rateds.rate-3)*recommendations.is_similar) > 7 AND sum(rateds.rate)*20 DIV COUNT(movies.id) > 80 AND sum(IF(r2.id IS NULL OR r2.rate = 0, 0, 1)) = 0')
        ->orderBy('point', 'desc')
