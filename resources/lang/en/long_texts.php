...
return [
    ...
    'notifications' => array(
      'air_date' => "<a ng-href=\"series/{{notification.data[0].movie_id}}\" class=\"text-dark\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"{{notification.data[0].original_title}}\">{{notification.data[0].title}} {{notification.data[0].release_date.length>0?'('+notification.data[0].release_date.substring(0, 4)+')':''}}</a> new episode air date is defined. Date: {{notification.data[0].next_episode_air_date.substring(0, 10)}} ({{notification.data[0].day_difference_next}} {{notification.data[0].day_difference_next>1?'days':'day'}} later)",
      'like' => "{{notification.total}} {{notification.total>1?'users':'user'}} liked your <a ng-href=\"{{notification.data[0].review_mode==1?'movie':'series'}}/{{notification.data[0].movie_id}}\" class=\"text-dark\" ng-if=\"notification.data[0].notification_mode==0\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"{{notification.data[0].original_title}}\">{{notification.data[0].title}} {{notification.data[0].release_date.length>0?'('+notification.data[0].release_date.substring(0, 4)+')':''}} review</a><a ng-href=\"list/{{notification.data[0].list_id}}\" class=\"text-dark\" ng-if=\"notification.data[0].notification_mode==1\">{{notification.data[0].title}} list</a>. {{notification.total>1?'Users':'User'}}: <span ng-repeat=\"item in notification.data\"><span ng-hide=\"&dollar;index==0\">, </span><a href=\"profile/{{item.user_id}}\" class=\"text-dark\">{{item.user_name}}</a></span>",
      'sent_movie' => "{{notification.total}} {{notification.total>1?'users':'user'}} recommended <a ng-href=\"{{notification.data[0].notification_mode==4?'movie':'series'}}/{{notification.data[0].movie_id}}\" class=\"text-dark\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"{{notification.data[0].original_title}}\">{{notification.data[0].title}} {{notification.data[0].release_date.length>0?'('+notification.data[0].release_date.substring(0, 4)+')':''}}</a>. {{notification.total>1?'Users':'User'}}: <span ng-repeat=\"item in notification.data\"><span ng-hide=\"&dollar;index==0\">, </span>{{item.user_name}}</span>",
      'watch_together' => "{{notification.data[0].user_name}} watched together with you. You can send movies to this user. If you want to know about this user, <a href=\"recommendations/notification.data[0].user_id\" class=\"text-dark\">watch together</a> back.",
    )
];