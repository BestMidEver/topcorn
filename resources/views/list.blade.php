...
<div class="row no-gutters" ng-cloak>
	...
	<div class="col-12 col-lg-10 col-xl-8">
		...
		<div class="container-fluid mt-3">
				...
				<div class="col-12">
					<div class="card h-100">
						<div class="d-flex flex-wrap justify-content-between">
							<div class="p-1">
								<a ng-href="/@{{movie.mode==0?'movie':'series'}}/@{{movie.id}}" ng-attr-id="movie-@{{movie.id}}" data-toggle="tooltip" data-placement="top" title="@{{movie.original_title}}" target={{$target}}>
									<h6 class="text-dark p-1 text-hover-underline">
										@if($liste[0]->sort != 0)
										<span>@{{ movie.position }}.</span> 
										@endif
										@{{ movie.title }} <small class="text-muted" ng-show="movie.release_date.length > 0"><em>(@{{movie.release_date.substring(0, 4)}})</em></small></h6>
								</a>
							</div>
							@if(Auth::check())
							...
							<div class="row no-gutters pt-3 pt-md-2">
								<div class="col-4 col-md-3 col-xl-2">
									<a ng-href="/@{{movie.mode==0?'movie':'series'}}/@{{movie.id}}" ng-mouseover="show_tooltip('movie-'+movie.id)" ng-mouseleave="hide_tooltip('movie-'+movie.id)()" target={{$target}}>
								...
								<div class="col-8 col-md-9 col-xl-10 lead lead-small" ng-show="!movie.explanation && movie.overview.length > 0">
									<div class="pr-1 pb-1 text-dark quote-line">@{{ movie.overview }}</div>
								</div>
								<div class="col-8 col-md-9 col-xl-10 lead lead-small" ng-show="movie.explanation">
			...
			<div class="mt-5 d-flex flex-row justify-content-between">
				<div class="d-flex flex-column">
					<a href="/profile/{{ $liste[0]->user_id }}#!#Lists" class="text-no-decoration" target={{$target}}>
				...
				<div class="d-flex flex-column mt-3 ml-1 fa22">
					<div class="d-flex flex-row">
						<button class="btn btn-outline-secondary btn-sm btn-block border-0 mt-0 px-lg-4 addseen opacity-1" ng-class="{'text-success':is_liked}" ng-click="like_list()" ng-disabled="{{(Auth::check()==0 || $liste[0]->user_id==Auth::id())?'true':'false'}}">
		...
		<!--<div class="container-fluid px-0 pt-5">
			...
			<div class="fb-comments" data-href="https://topcorn.xyz/list/1" data-width="100%" data-numposts="6" data-colorscheme="{{Auth::check()?(Auth::User()->theme==1?'dark':'light'):''}}"></div>
		</div>-->
	</div>
	<div class="col"></div>
</div>
