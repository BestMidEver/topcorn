...
<div class="col mt-1 mb-2 mt-md-4">
	...
	<div class="d-inline ml-3" ng-show="active_tab=='mood_pick'">
		...
		<button class="btn btn-outline-warning btn-sm my-2" type="button" data-toggle="collapse" data-target="#collapseAdd" ng-click="setFocus('input_mode')"><i class="fas fa-plus"></i> <span ng-show="mode_movies.length == 0">{{ __('general.pick_mode') }}</span><span ng-hide="mode_movies.length == 0"> {{ __('general.add_movie') }}</span></button>
	</div>
</div>
<!-- H1 +AddPerson +AddMode row  -->
...
<div class="container-fluid mt-3 pb-1" id="filter">
	<ul class="nav justify-content-md-center tab1">
		...
		<li class="nav-item ml-3 pb-2">
			<button class="nav-link btn btn-outline-secondary btn-sm" ng-click="drawslider();scroll_to_filter()" type="button" data-toggle="collapse" data-target="#collapseFilter" ng-disabled="{{ auth::check()?'false':'true' }} && active_tab=='top_rated'" ng-click="is_filter_collapsed()"><i class="fa fa-filter"></i> {{ __('general.filter') }}</button>
		</li>
	</ul>
</div>
<!-- Tabs and Filter Button -->
