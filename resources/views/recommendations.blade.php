...
<!-- Tabs and Filter Button Mobile -->
	<div class="scrollmenu d-md-none tab2">
		...
		<button class="btn btn-link border-no-radius flex-sm-fill text-sm-center text-muted text-no-decoration" ng-class="{'active':active_tab=='pemosu'}" ng-click="active_tab='pemosu';get_first_page_data()" {{ auth::check()?null:'disabled' }}>{{ __('general.according_to_my_taste') }}</button>
