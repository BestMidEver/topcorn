						...
						<div class="d-flex flex-row justify-content-center" ng-if="!isNaN(movie.day_difference_last)">
							<div class="text-white" ng-if="(movie.day_difference_last<1 || !movie.last_episode_air_date.length>0) && movie.last_seen_air_date.length>0">
								...
								<span class="d-block">
									...
									<span ng-if="movie.day_difference_last==0 && !next_episode>0" class="h5 text-warning"><span ng-if="movie.status=='Ended'">There won't be any(Ended)</span><span ng-if="movie.status=='Canceled'">There won't be any(Canceled)</span></span>
								</span>
							</div>
