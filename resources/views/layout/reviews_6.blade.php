<div class="container-fluid" ng-cloak>
	<div class="py-4" ng-repeat="review in reviews{{ $suffix }} | limitTo:3">
        <div class="d-flex justify-content-between">
            ...
            <div class="d-inline" ng-if="review.name.length>0"><a class="h6 pb-2 text-dark d-inline" ng-href="/@{{review.mode==1?'movie':'series'}}/@{{review.movie_series_id}}" data-toggle="tooltip" data-placement="top" title="@{{review.original_title}}">@{{review.movie_title}} <small class="text-muted" ng-if="review.release_date.length > 0"><em>(@{{review.release_date.substring(0, 4)}})</em></small><span ng-if="review.season_number!=null"> • S@{{review.season_number>9?review.season_number:'0'+review.season_number}}</span><span ng-if="review.episode_number!=null">E@{{review.episode_number>9?review.episode_number:'0'+review.season_number}}</span></a> <em class="mx-2 mx-md-3">|</em> <a class="h6 pb-2 text-dark d-inline" ng-href="/profile/@{{review.user_id}}">@{{review.name}}</a> <span class="ml-2 scrollmenu pb-2" ng-if="review.rate>0"><i class="fas fa-star" ng-class="{1:'text-danger', 2:'text-warning', 3:'text-secondary', 4:'text-info', 5:'text-success'}[review.rate]" ng-repeat="n in [] | range:review.rate"></i><i class="far fa-star text-muted" ng-repeat="n in [] | range:(5-review.rate)"></i></span></div>
            <div class="h6">
	...
	<div ng-if="reviews{{ $suffix }}.length > 3">
    	<div class="py-4" ng-repeat="review in reviews{{ $suffix }} | limitTo:100:3">
            <div class="d-flex justify-content-between">
                ...
                <div class="d-inline" ng-if="review.name.length>0"><a class="h6 pb-2 text-dark d-inline" ng-href="/@{{review.mode==1?'movie':'series'}}/@{{review.movie_series_id}}" data-toggle="tooltip" data-placement="top" title="@{{review.original_title}}">@{{review.movie_title}} <small class="text-muted" ng-if="review.release_date.length > 0"><em>(@{{review.release_date.substring(0, 4)}})</em></small><span ng-if="review.season_number!=null"> • S@{{review.season_number>9?review.season_number:'0'+review.season_number}}</span><span ng-if="review.episode_number!=null">E@{{review.episode_number>9?review.episode_number:'0'+review.season_number}}</span></a> <em class="mx-2 mx-md-3">|</em> <a class="h6 pb-2 text-dark d-inline" ng-href="/profile/@{{review.user_id}}">@{{review.name}}</a> <span class="ml-2 scrollmenu pb-2" ng-if="review.rate>0"><i class="fas fa-star" ng-class="{1:'text-danger', 2:'text-warning', 3:'text-secondary', 4:'text-info', 5:'text-success'}[review.rate]" ng-repeat="n in [] | range:review.rate"></i><i class="far fa-star text-muted" ng-repeat="n in [] | range:(5-review.rate)"></i></span></div>
                <div class="h6">
                    <button class="btn btn-outline-secondary btn-sm border-0 mt-0 addseen opacity-1" ng-disabled="review.is_mine==1||review.is_mine==undefined" ng-click="like_review($index+3)">
                        <div ng-class="{'text-success':review.count>0}"><i class="fa-heart" ng-class="{0:'far', 1:'fas', undefined:'far'}[review.is_liked]"></i><span ng-if="review.count>0"> @{{review.count}}</span></div>
                    </button>
                </div>
            </div>
            <div id="@{{'accordion'+($index+3)}}">
                <div ng-if="review.id == 'long'">
                    <div id="@{{'collapse'+($index+3)+'a'}}" data-parent="@{{'#accordion'+($index+3)}}" class="lead lead-small collapse">
                        ...
                        <div class="text-center pt-0">
                            <button class="btn btn-outline-secondary btn-lg fa40 border-0 text-muted hover-white hidereview" data-toggle="collapse" data-target="@{{'#collapse'+($index+3)+'b'}}" aria-expanded="true"><i class="fa fa-angle-up"></i></button>
                        </div>
                    </div>
                </div>
                <div>
