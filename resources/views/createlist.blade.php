						...
						<div class="row no-gutters mt-3">
							...
							<div class="input-group col mt-3 mt-md-0">
								<div class="input-group-prepend pl-md-3">
									<button class="btn btn-outline-secondary" ng-class="{'active':movie.mode!=1}" ng-click="movie.mode=0;search_movie(movie);set_focus($index)" type="button">Movies</button>
								...
								<div ng-show="(movie.search || movie.choosing) && movie.movies.length > 0 && movie.searchmode" class="search-movie-results background-white py-3" ng-mouseenter="movie.choosing=true" ng-mouseleave="movie.choosing=false">
									<div class="result py-1" ng-repeat="f in movie.movies" ng-click="choose_movie($parent.$index, f)">@{{movie.mode!=1?f.title:f.name}} <small ng-if="f.release_date.length > 0"><em>(@{{f.release_date.substring(0, 4)}})</em></small><small ng-if="f.first_air_date.length > 0"><em>(@{{f.first_air_date.substring(0, 4)}})</em></small></div>
								</div>
							</div>
						</div>
